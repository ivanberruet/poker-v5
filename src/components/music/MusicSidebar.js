...
export default function MusicSidebar({view, setView}) {
    ...
    <div className='text-neutral-400 p-5 text-xs lg:text-sm border-r border-neutral-900 overflow-y-scroll scrollbar-hidden h-screen hidden md:block w-full sm:max-w-[12rem] lg:max-w-[15rem] pb-36'>
      <div className='space-y-4'>
        ...
        <div className='flex flex-col gap-4'>
          <button onClick={() => setView('search')} className={`flex gap-2 hover:text-white cursor-pointer  ${view === 'search' && 'text-white'}`}>
            <FontAwesomeIcon icon={faMagnifyingGlass} className='w-5 h-5' />
            <p>Buscar</p>
          </button>

          <button onClick={() => setView('library')} className={`flex gap-2 hover:text-white cursor-pointer  ${view === 'library' && 'text-white'}`}>
